...
override IMAGE_NAME := fangaos-$(KARCH)
...
OVMF_DIR := ovmf
OVMF_TARBALL := $(OVMF_DIR)/edk2-ovmf.tar.gz
OVMF_URL := https://github.com/osdev0/edk2-ovmf-nightly/releases/download/nightly-20251213T012300Z/edk2-ovmf.tar.gz
OVMF_EXTRACTED := $(OVMF_DIR)/edk2-ovmf
$(OVMF_TARBALL):
	mkdir -p $(OVMF_DIR)
	curl -L -o $@ $(OVMF_URL)

$(OVMF_EXTRACTED): $(OVMF_TARBALL)
	rm -rf $(OVMF_EXTRACTED)
	tar -xzf $(OVMF_TARBALL) -C $(OVMF_DIR)
	@test -f $(OVMF_EXTRACTED)/ovmf-code-x86_64.fd


ovmf/ovmf-code-$(KARCH).fd: $(OVMF_EXTRACTED)
	cp -v $(OVMF_EXTRACTED)/ovmf-code-$(KARCH).fd $@

ovmf/ovmf-vars-$(KARCH).fd: $(OVMF_EXTRACTED)
	cp -v $(OVMF_EXTRACTED)/ovmf-vars-$(KARCH).fd $@
...
$(IMAGE_NAME).iso: limine/limine kernel
	...
	cp -v kernel/target/x86_64-fanga-kernel/release/fanga-kernel iso_root/boot/kernel
...
ifeq ($(KARCH),x86_64)
	...
	mcopy -i $(IMAGE_NAME).hdd@@1M kernel/target/x86_64-fanga-kernel/release/fanga-kernel ::/boot/kernel
